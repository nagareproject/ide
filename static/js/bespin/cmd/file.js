...
bespin.command.store.addCommand({
    ...
    execute: function(instruction, revision) {
        ...
        if (revision == "on") {
            var url = bespin.util.path.combine('/history/at', session.project, session.path);
            ...
            // display
            var session = bespin.get("editSession");
            var url = bespin.util.path.combine('/file/at', session.project, session.path);
    ...
    historyToElement: function(history) {
        ...
        history.forEach(function(entry) {
            ...
            dojo.create("td", {
                innerHTML: bespin.util.formatDate(new Date(entry.date * 1000))
            }, row);
